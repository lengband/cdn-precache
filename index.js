...
getip.get('https://api.smartproxy.cn/web_v1/ip/get-ip-v3?app_key=5d884abaf2ac978d71f6e2c9987e1508&pt=9&num=10&ep=&cc=nl&state=&city=&life=30&protocol=1&format=json&lb=%5Cr%5Cn')
  .then((res) => {
    // res.data 是 类似下面的数据
    ...
    const testUrl = 'https://www.okx.com/cdn/assets/okfe/inner/assets-system-test/0.0.5/index.css';
    ...
    agentList.forEach(async (item, index) => {
      ...
      instance.get(testUrl)
        .then(response => {
          console.log({ status: response.status, CloudflareHit: response.headers['cf-cache-status'], statusText: response.statusText });
          if (index === 0) {
